...
const EXPORT_DEFINITIONS: ExportRecord[] = [
	{
		name: "hero",
		filename: "landing-hero.json",
		payload: landingContentGaps.hero,
	},
	{
		name: "hero-fallback",
	...
	{
		name: "faq-cta",
		filename: "landing-faq-cta.json",
		payload: landingContentGaps.faqCta,
	},
	{
		name: "pricing-header",
	...
	{
		name: "feature-ideas",
		filename: "upcoming-features.json",
		payload: mockFeatures,
	},
];
...
const resolveExportRecords = (entities?: string[]): ExportRecord[] => {
	...
	const selected = EXPORT_DEFINITIONS.filter((record) =>
		requested.has(record.name),
	);
...
export async function exportLandingData(
	...
	for (const record of records) {
		...
		if (!silent) {
			...
			console.log(
				`✓ exported ${record.name} -> ${path.relative(process.cwd(), destination)}`,
			);
		}
	}
