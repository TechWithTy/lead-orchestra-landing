...
export function getBlogSeo(post: BeehiivPost): SeoMeta {
	...
	let description = post.meta_default_description || post.preview_text || defaultSeo.description;
	if (
		!description &&
		post.content &&
	...
	const dateModified =
		resolveBeehiivDate(post.displayed_date, post.publish_date, post.published_at) || datePublished;
