...
const AUTHORITY_REVIEW_SCHEMAS: ReviewSchema[] = [
	...
	},
	{
		...
		reviewBody: 'The voice cloning and CRM sync save us hours daily. Onboarding took minutes.',
		author: {
			'@type': 'Person',
			name: 'Wholesaler',
		},
...
export const buildOrganizationSchema = (): OrganizationSchema => {
	...
	const allReviews =
		reviews.length > 0 ? [...AUTHORITY_REVIEW_SCHEMAS, ...reviews] : AUTHORITY_REVIEW_SCHEMAS;
...
export const buildSoftwareApplicationSchema = (): SoftwareApplicationSchema => ({
	'@context': SCHEMA_CONTEXT,
...
export const buildDatasetSchema = (): DatasetSchema => ({
	...
	keywords: ['real estate', 'lookalike audience', 'ai prospecting', 'deal sourcing'],
	distribution: [
		{
			'@type': 'DataDownload',
...
export const buildServiceSchema = (service: ServiceSchemaInput): ServiceSchema => ({
	'@context': SCHEMA_CONTEXT,
	'@type': 'Service',
...
export const buildProductSchema = (product: ProductSchemaInput): ProductSchema => ({
	'@context': SCHEMA_CONTEXT,
	'@type': 'Product',
	...
	offers: {
		...product.offers,
		'@type': 'Offer',
		url: product.offers.url ? buildAbsoluteUrl(product.offers.url) : product.offers.url,
	},
	manufacturer: {
		'@id': ORGANIZATION_ID,
	},
...
const normalizeIsoDate = (value?: number | string | Date | null): string | undefined => {
	...
	if (typeof value === 'number') {
		...
		return Number.isNaN(dateFromNumber.getTime()) ? undefined : dateFromNumber.toISOString();
	}
...
const buildCaseStudyReviews = (caseStudy: CaseStudy, caseStudyUrl: string): ReviewSchema[] => {
...
export const buildCaseStudyCreativeWorkSchema = (
	...
	return {
		...
		image: caseStudy.featuredImage ? buildAbsoluteUrl(caseStudy.featuredImage) : undefined,
