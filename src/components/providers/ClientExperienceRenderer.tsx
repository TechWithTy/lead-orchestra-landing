 ...
 */
export function ClientExperienceRenderer({ props }: { props: ClientExperienceProps }) {
	// During SSR, return null immediately - never evaluate anything
	// This check MUST be first, before any hooks
	if (typeof window === 'undefined') {
		console.log('[ClientExperienceRenderer] SSR detected - returning null immediately');
		return null;
	}
	...
	useEffect(() => {
		...
		import('./ClientExperienceWrapper')
			...
			.catch((error) => {
				console.error('[ClientExperienceRenderer] Failed to load component:', error);
			});
	}, []);
