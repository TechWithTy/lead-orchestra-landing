			...
			<div>
				...
				<div className="flex items-center space-x-1">
					<a
						...
						target="_blank"
						rel="noreferrer"
						className="text-gray-500 text-sm transition-all duration-75"
					>
		...
		{tweet.entities.map((entity, idx) => {
			switch (entity.type) {
				...
				case 'mention':
					return (
						<a
							key={`tweet-entity-${entity.type}-${entity.text}-${idx}`}
							href={entity.href}
							target="_blank"
				...
				case 'text':
					return (
						<span
							key={`tweet-text-${entity.type}-${entity.text}-${idx}`}
							// biome-ignore lint/nursery/useSortedClasses: <explanation>
...
export const MagicTweet = ({
	...
	return (
		<div
			className={cn(
				'relative flex size-full max-w-lg animate-fade-in flex-col gap-2 overflow-hidden rounded-lg border p-4 backdrop-blur-md',
				className
			)}
			{...props}
		>
