...
const sanitizeCopy = (copy: AbTestCopyInput): ABTestCopy => {
	const trimmed = Object.entries(copy).reduce<Partial<ABTestCopy> & Record<string, unknown>>(
		(acc, [key, value]) => {
	...
	const hopeSource =
		...
		(typeof trimmed.whatsInItForMe === 'string' ? trimmed.whatsInItForMe : undefined) ??
		(trimmed.solution && trimmed.solution.length > 0 ? trimmed.solution : undefined) ??
		'';

	const fearSource =
		(trimmed.fear && trimmed.fear.length > 0 ? trimmed.fear : undefined) ??
		(trimmed.pain_point && trimmed.pain_point.length > 0 ? trimmed.pain_point : undefined) ??
		'';
...
const analyzeVariants = (
	...
	const totalPercentage = variants.reduce((total, variant) => total + (variant.percentage ?? 0), 0);
...
const defineAbTests = (tests: AbTestInput[]): ABTest[] => tests.map((item) => defineAbTest(item));

export const abTestExample = defineAbTest({
	id: 'abtest-001',
	name: 'Product Sales Copy Test',
	description: 'A/B test for optimizing product sales copy messaging for modern creators.',
	variants: [
		{
			name: 'Variant A',
			percentage: 50,
			copy: {
				...
				whatsInItForMe: "Feel confident and comfortable updatedx, whether you're at work or play.",
				...
				additionalInfo: 'Limited-time offer: Free shipping on your first order!',
			},
		...
		},
		{
			...
			copy: {
				...
				description: 'Experience the difference with our advanced fabric blend and ergonomic fit.',
			...
			variant_description: 'Challenger copy focusing on performance and sustainability.',
			kpis: [
...
export const AIConversationCreditsABTest = defineAbTest({
	...
	variants: [
		{
			...
			copy: {
				...
				tagline: "Your Scraping Pipeline is Ready to Process Leads. Don't Let It Run Out of Fuel.",
				subtitle:
			...
			variant_description: 'Emphasizes outcome-driven automation and the value of Lead Credits.',
			kpis: [
	...
	isActive: true,
	facebookPixelUsers: [],
	targetAudience: 'Growth-focused real estate investors, wholesalers, and flippers',
	kpis: [
		...
		variants: [
			{
				...
				copy: {
					...
					highlights: ['Close deals faster', 'Save on legal fees', 'Fully customizable templates'],
				},
			},
			{
		...
		variants: [
			{
				...
				copy: {
					...
					target_audience: 'Professionals who want a cleaner, more organized, and protected desk.',
					pain_point: 'A cluttered, scratched desk looks unprofessional and feels chaotic.',
					solution:
						'This large desk mat instantly organizes your workspace and protects it from spills and scratches.',
					highlights: ['Protects from scratches', 'Non-slip surface', 'Defines your workspace'],
				},
			},
			{
				...
				copy: {
					...
					subtitle: 'Add a touch of professional Deal Scale branding to your home or office desk.',
					whatsInItForMe:
					...
					highlights: ['Elevate your office look', 'Professional branding', 'Premium feel'],
				},
			},
		],
		startDate: new Date('2023-11-01T09:00:00.000Z'),
		...
		variants: [
			{
				...
				copy: {
					...
					highlights: ['Eliminate back pain', 'Increase your focus', 'Work comfortably for hours'],
				},
			},
			{
		...
		variants: [
			{
				...
				copy: {
					...
					highlights: ['Reduce eye strain', 'Improve late-night focus', 'Fully adjustable light'],
				},
			},
			{
				...
				copy: {
					...
					highlights: ['Sleek, modern design', 'Customizable ambiance', "Upgrade your desk's look"],
				},
			},
		],
		startDate: new Date('2023-11-01T09:00:00.000Z'),
		...
		variants: [
			...
			},
			{
				...
				copy: {
					...
					pain_point: 'A disorganized sales process leads to a slow sales cycle and lost revenue.',
					solution:
		...
		variants: [
			{
				...
				copy: {
					...
					target_audience: 'Founders who are currently or about to start fundraising.',
					pain_point:
