...
export async function GET() {
	try {
		...
		const debugResponse = await fetch(`${DEALSCALE_API_BASE}/api/v1/features/debug-test`, {
			method: 'GET',
			headers: {
		...
		if (!debugResponse.ok) {
			console.error(
				'Failed to call features debug test:',
		...
		console.error('Features debug test error:', error);
